...
import { Check, Eye, EyeOff, KeyRound, Lock, Plus, Save, Trash2 } from 'lucide-react'
import {
...
function blankProfile(id = `profile-${Date.now()}`): ModelApiProfile {
  return { id, name: 'New model API', provider: 'custom', model: '', apiKey: '', baseUrl: '' }
}
...
export default function ApiKey() {
  ...
  const chooseProvider = (provider: string) => {
    ...
    setDraft((current) => ({ ...current, provider, baseUrl: preset?.[2] || '' }))
  }

  const save = () => {
    if (!draft.model.trim() || !draft.apiKey.trim()) return
    const normalized = { ...draft, name: draft.name.trim() || providerName, model: draft.model.trim(), apiKey: draft.apiKey.trim(), baseUrl: draft.baseUrl.trim() }
  ...
  const deleteProfile = () => {
    if (draft.id === 'custom' && profiles.length === 0) return
    removeProfile(draft.id)
    const nextProfiles = getProfiles()
    setProfiles(nextProfiles)
    const next = nextProfiles[0] || blankProfile('custom')
      ...
      <div className="mx-auto max-w-7xl px-6">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <h2 className="font-display text-4xl font-bold leading-tight text-white sm:text-5xl md:text-6xl">Connect any model API.<br /><span className="text-zinc-500">No model aggregator required.</span></h2>
          </div>
          <p className="text-zinc-400 max-w-md">Store multiple provider profiles, choose the active model, and send its API key directly to ANT. Use a custom endpoint for any OpenAI-compatible model API.</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-4 space-y-3">
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/[0.04] p-5">
              <div className="flex items-center justify-between mb-4"><div><div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">Saved model APIs</div><div className="mt-1 text-lg font-semibold text-white">{profiles.length} configured</div></div><button type="button" onClick={addProfile} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10 hover:text-white" aria-label="Add model API"><Plus className="h-4 w-4" /></button></div>
              <div className="space-y-2">
                {profiles.map((profile) => <button type="button" key={profile.id} onClick={() => selectProfile(profile.id)} className={`w-full rounded-xl border px-3 py-3 text-left transition-all ${activeId === profile.id ? 'border-amber-500/40 bg-amber-500/10' : 'border-white/5 bg-white/[0.015] hover:bg-white/5'}`}><div className="truncate text-sm font-medium text-white">{profile.name || profile.provider}</div><div className="mt-1 truncate text-[10px] font-mono uppercase tracking-widest text-zinc-500">{profile.provider} · {profile.model || 'model not set'}</div></button>)}
                {profiles.length === 0 && <div className="rounded-xl border border-dashed border-white/10 p-4 text-xs text-zinc-500">No API profiles yet. Add one to connect a model.</div>}
              </div>
            </div>
            <div className="rounded-2xl border border-white/5 bg-white/[0.015] p-5"><div className="flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-zinc-500"><Lock className="h-3.5 w-3.5 text-emerald-400" /> Credential handling</div><ul className="mt-3 space-y-2 text-xs text-zinc-400"><li>• Keys stay in browser local storage until you send a request.</li><li>• ANT forwards the selected key only for the current request.</li><li>• Provider and endpoint are selected explicitly; no hidden routing.</li></ul></div>
          </div>

          <div className="lg:col-span-8 relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.02] backdrop-blur-sm">
            <div className="flex items-center justify-between border-b border-white/5 bg-white/[0.02] px-5 py-3"><div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/15 text-amber-400"><KeyRound className="h-4 w-4" /></div><div><div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">Active model API</div><div className="text-sm font-semibold text-white">{providerName}</div></div></div><button type="button" onClick={deleteProfile} className="inline-flex items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-[10px] font-mono uppercase tracking-widest text-zinc-400 hover:text-rose-300"><Trash2 className="h-3 w-3" /> Remove</button></div>
