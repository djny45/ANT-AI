...
const seedMessages: Message[] = [
  ...
  { id: 'm1', role: 'ant', text: 'NOVA Core native mode is available. External model APIs are optional.', meta: 'Queen · t=0ms' },
]
...
export default function ChatBox() {
  ...
  const send = async (text?: string) => {
    ...
    const nativeNova = selected.provider === 'nova-core'
    if (!selected.model.trim() || (!nativeNova && !selected.apiKey.trim())) {
      setMessages((m) => [...m, { id: `e-${Date.now()}`, role: 'system', text: nativeNova ? 'NOVA Core is selected but no model is configured.' : 'Configure a model and API key in the API Configuration panel first.', meta: 'configuration' }])
      return
    }
    ...
    try {
      ...
      const runtime = result?.runtime || (nativeNova ? 'nova-core' : 'provider-api')
      setMessages((m) => [...m, { id: `a-${Date.now()}`, role: 'ant', text: String(responseText), meta: `ANT API · ${runtime} · ${selected.model}` }])
    } catch (error) {
  ...
  return <section id="chat" className="relative w-full bg-[#0a0a0a] py-28 sm:py-36 border-t border-white/5"><div className="mx-auto max-w-7xl px-6">
    <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-12 flex flex-col md:flex-row md:items-end md:justify-between gap-6"><div className="max-w-2xl"><div className="mb-3 inline-flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-amber-400"><span className="h-px w-6 bg-amber-500" /> Talk to the Queen</div><h2 className="font-display text-4xl font-bold leading-tight text-white sm:text-5xl md:text-6xl">Try the swarm.<br /><span className="text-zinc-500">Right here, right now.</span></h2></div><p className="text-zinc-400 max-w-md">Use NOVA Core natively, or choose any configured external model API in the panel. ANT never silently switches models.</p></motion.div>
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div className="lg:col-span-3 space-y-2"><div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 px-1">Nano capabilities</div>{[{ label: 'Development', state: 'ready' }, { label: 'Repair', state: 'ready' }, { label: 'Analysis', state: 'ready' }, { label: 'Research', state: 'standby' }, { label: 'Audit', state: 'ready' }].map((c, i) => <motion.div key={c.label} initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 + i * 0.05 }} className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.015] px-3 py-2.5"><div className="flex items-center gap-2"><Cpu className="h-3.5 w-3.5 text-zinc-500" /><span className="text-xs text-zinc-200">{c.label}</span></div><div className="flex items-center gap-1.5"><span className={`h-1.5 w-1.5 rounded-full ${c.state === 'ready' ? 'bg-emerald-400 pulse-dot' : 'bg-zinc-600'}`} /><span className="font-mono text-[9px] uppercase tracking-widest text-zinc-500">{c.state}</span></div></motion.div>)}<div className="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/[0.03] p-4"><div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">Selected runtime</div><div className="mt-2 truncate font-mono text-xs text-amber-300" title={`${profile.provider} · ${profile.model}`}>{profile.provider || 'nova-core'} · {profile.model || 'not configured'}</div></div><div className="rounded-2xl border border-white/5 bg-white/[0.015] p-4"><div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">Session</div><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><div><div className="text-zinc-500">Messages</div><div className="font-mono text-white">{messages.length}</div></div><div><div className="text-zinc-500">Latency</div><div className="font-mono text-white">API</div></div></div></div></div>
